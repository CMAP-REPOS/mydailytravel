...
avgtravel_mdt <-
  ...
  filter(out_tt_trip == 0) %>% # 95272 records
...
travel_summaries <-
  ...
  select(type,subtype,trips_per_capita,avg_trip_length,distance_per_capita,survey) %>% 
...
average_resident_p1 <-
  ...
  geom_vline(data = travel_summaries_vlines %>% filter(survey == "mdt"),
